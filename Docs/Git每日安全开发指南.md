...
2. **安全第一**：引入 `backup2` 分支作为“防爆盾”，所有合并必须先在备份分支验证，确保无误后再合入主干。

------

### 0. 首次配置 (One-time Setup)

如果你本地还没有 `backup2` 分支，请先执行以下命令将其拉取到本地：

Bash

```
git fetch origin
git checkout -b backup2 origin/backup2
# 此时你本地应该同时拥有 main 和 backup2 两个分支
```
...
**非常重要**：每天开始写代码前，必须确保你的 `main` 和 `backup2` 都是最新的，避免合并冲突。
...
永远不要在 `main` 或 `backup2` 上直接写代码。任何修改（新功能、修Bug、改资源）都要切新分支。
...
开发完成后，**不要直接合入 main**。我们需要先在 `backup2` 上“试错”。
...
只有在 `backup2` 上合并成功且项目运行正常后，才执行此步。
...
4. **实合 (Main)**：
   - `git checkout main`
   - `git merge --no-ff fix/bug-1 -m "......"`
   - `git push`
5. 更新备份（如果没有问题）
   - `git checkout backup2`
   - `git push`
6. **收尾**：`git branch -d fix/bug-1` (删除本地功能分支)
