...
namespace Constants {
    ...
    // 地图配置 (PS:不用Map命名空间以避免与cocos2d::Map冲突)
    ...
    constexpr float ROOM_CENTER_DIST = 1248.0f; // 房间中心像素距离
    constexpr int DOOR_WIDTH = 4;              // 门宽度(地板块数)
    ...
    namespace ZOrder {
        ...
        constexpr int WALL_ABOVE = 60;     // 玩家上方的墙
        constexpr int EFFECT = 70;
        constexpr int MINIMAP = 90;
        constexpr int UI = 100;
        constexpr int UI_GLOBAL = 500;     // UI元素的全局Z序
    }
